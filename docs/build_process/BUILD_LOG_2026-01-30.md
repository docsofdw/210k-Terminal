...
### 6. Google Sheets Fallback Implementation

**Change:** Added automatic fallback to Google Sheets when APIs fail.

**How it works:**
1. `sync-market-data` tries MarketData.app (US) or Yahoo Finance (international) first
2. If API fails for a company, automatically fetches that company's data from Google Sheets
3. Logs which companies needed fallback for monitoring

**Cron changes:**
- `sync-sheets` removed from scheduled crons (was every 4 hours)
- Endpoint still available at `/api/cron/sync-sheets` for manual use
- Portfolio syncs (`sync-portfolio`, `sync-fund-performance`, `sync-live-fund-stats`) unchanged

**Response includes fallback stats:**
```json
{
  "updated": 65,
  "fallbackUsed": 3,
  "fallbackTickers": ["TRSR", "BTCB.ST"],
  "skipped": 2
}
```
...
### APIs (Primary for Market Data)
| Data | Provider | Frequency |
|------|----------|-----------|
| US stock prices | MarketData.app | 15 min |
| International stock prices | Yahoo Finance | 15 min |
| Balance sheet (cash, debt) | Twelve Data | Daily |
| BTC price | CoinGecko | 5 min |
| FX rates | Exchange Rate API | 4 hours |

### Google Sheets (Primary for Portfolio)
| Data | Sheet | Frequency |
|------|-------|-----------|
| Fund positions | Live Portfolio | 4 hours |
| Fund performance | Fund Performance | 1st & 15th |
| Live NAV | Fund Stats | 4 hours |

### Google Sheets (Fallback for Market Data)
| Data | When Used |
|------|-----------|
| Company prices/mNAV | When API fails for specific company |
...
4. Monitor fallback usage - high fallback rates may indicate API issues
